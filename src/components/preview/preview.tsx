      ...
      component: (
        <section className={s.section}>
          ...
          <div className={s.section__fields}>
            {skills.length > 0 ? (
              <ul>
                {skills.map((el, index) => <li key={`${index}-${el}`}>- {el}</li>)}
              </ul>
        ...
        <section className={s.section}>
          ...
          <div className={s.section__fields}>
            {certificates.length > 0 ? (
              <ul>
                {certificates.map((el, index) => <li key={`${index}-${el}`}>- {el}</li>)}
              </ul>
...
export const Preview = () => {
  ...
  const sectionsMap: Record<string, SectionComponent> = {
    'exp-1': { 
      id: 'exp-1', 
      component: (
        <section className={s.section} key="exp-1">
          <h3>Опыт</h3>
          <div className={s.section__fields}>
            <div className={s.section__fields_item}><span>Должность:</span> {experience.position || 'Не указано'}</div>
            <div className={s.section__fields_item}><span>Компания:</span> {experience.company || 'Не указано'}</div>
    ...
    'edu-1': { 
      id: 'edu-1', 
      component: (
        <section className={s.section} key="edu-1">
          <h3>Образование</h3>
          <div className={s.section__fields}>
            <div className={s.section__fields_item}><span>Учебное заведение:</span> {education.collage || 'Не указано'}</div>
            <div className={s.section__fields_item}><span>Специальность:</span> {education.major || 'Не указано'}</div>
    ...
    'skills-1': { 
      id: 'skills-1', 
      component: (
        <section className={s.section} key="skills-1">
          <h3>Навыки</h3>
          <div className={s.section__fields}>
            {skills.length > 0 ? (
              <ul>
                {skills.map((el, index) => (
                  <li key={`skill-${index}`}>- {el}</li>
                ))}
              </ul>
            ) : <p>Навыки не указаны</p>}
          </div>
    ...
    'certs-1': { 
      id: 'certs-1', 
      component: (
        <section className={s.section} key="certs-1">
          <h3>Сертификаты</h3>
          <div className={s.section__fields}>
            {certificates.length > 0 ? (
              <ul>
                {certificates.map((el, index) => (
                  <li key={`cert-${index}`}>- {el}</li>
                ))}
              </ul>
            ) : <p>Сертификаты не указаны</p>}
          </div>
    ...
    'about-1': { 
      id: 'about-1', 
      component: (
        <section className={s.section} key="about-1">
          <h3>Обо мне</h3>
          <div className={s.section__fields}>
            {aboutMe ? <p>{aboutMe}</p> : <p>Нет информации</p>}
          </div>
  ...
  const orderedSections = sectionsOrder
    .map(sectionId => sectionsMap[sectionId])
    .filter((section): section is SectionComponent => !!section);
